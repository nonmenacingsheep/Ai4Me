...
def author_building_messages(p: dict, ctx: dict) -> tuple[str, str]:
    ...
    system = (
        ...
        "  MASONRY — grander materials, ONLY if your people work them (see what they can build with):\n"
        "    S = stone wall   K = brick wall   P = stone pillar (inside)   G = glazed window\n"
        "    M = marble floor   N = grand gateway (like a door, but stately)\n"
        ...
        "RULES: keep it buildable — at MOST 12 rows by 12 columns. Wall the floor in. Put at LEAST one "
        "door (D) or gateway (N) on an outer edge so people can enter, and make every inside tile "
        "reachable from it. Match size to function — a workshop or store is small, a hall or keep "
        "larger and grander. Use the MASONRY glyphs only if your people can work stone/brick/glass; "
        "invent no materials beyond the glyphs listed."
    )
    user = (
